...
export default function DetailBannersPage() {
  ...
  // Filter detail banners
  const detailBanners = banners.filter((b) => b.type === "detail");
  const filteredBanners = detailBanners.filter((b) =>
    ...
    <div className="space-y-6 animate-in fade-in duration-300">
      ...
      <div>
        <h2 className="text-xl sm:text-2xl font-black text-slate-800 tracking-tight">
          Quản lý Banner Trang chi tiết
        </h2>
        <p className="text-xs text-slate-500 mt-1">
          Thiết lập banner phụ ở các trang chi tiết gói tour, chi tiết bài viết (Sidebar Banner hoặc Section Banner). Kích thước khuyến nghị: 800x400px hoặc 400x400px.
        </p>
      </div>
        ...
        <div className="lg:col-span-4">
          <div className="bg-white rounded-2xl border border-slate-100 p-6 shadow-sm sticky top-24">
            <h3 className="text-sm font-black text-slate-800 uppercase tracking-widest mb-4">
              {editingBanner ? "Cập nhật Banner" : "Thêm Banner Chi tiết"}
            </h3>
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="space-y-1.5">
                ...
                <input
                  ...
                  placeholder="Ví dụ: Ưu đãi hè 30%..."
              ...
              <div className="space-y-1.5">
                <label className="text-xs font-extrabold uppercase tracking-widest text-slate-500 block">
                  Thông điệp phụ (Subtitle)
                </label>
                <input
                  type="text"
                  value={subtitle}
                  onChange={(e) => setSubtitle(e.target.value)}
                  placeholder="Ví dụ: Đặt ngay hôm nay..."
                  className="w-full px-4 py-2.5 rounded-xl border border-slate-200 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-600 transition-all font-semibold text-slate-700"
                />
              </div>
              ...
              <div className="space-y-1.5">
                ...
                <input
                  ...
                  placeholder="Ví dụ: /detail-banner.jpg hoặc link online..."
        ...
        <div className="lg:col-span-8 space-y-4">
          ...
          <div className="bg-white rounded-2xl border border-slate-100 p-4 shadow-sm flex flex-col sm:flex-row gap-4 items-center justify-between">
            <h4 className="text-sm font-black text-slate-800 uppercase tracking-widest">
              Danh sách Banner ({detailBanners.length})
            </h4>
                ...
                <tbody className="divide-y divide-slate-50 text-slate-700 text-sm">
                  {filteredBanners.length === 0 ? (
                    <tr>
                      ...
                      >
                        Không tìm thấy banner trang chi tiết nào.
                      </td>
                    </tr>
                  ) : (
