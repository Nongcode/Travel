...
export default function PackageOffersPage() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (editingPromo) {
      updatePromoPackage(editingPromo.id, {
        ...promoData,
    ...
    <div className="space-y-6 animate-in fade-in duration-300">
      ...
      <div>
        ...
        <p className="text-xs text-slate-500 mt-1">
          Cài đặt các chương trình giảm giá, mã khuyến mại cho các tour du lịch để kích cầu du khách quốc tế.
        </p>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        {/* Left Form: Add/Edit Promo */}
        ...
        <div className="lg:col-span-8 space-y-4">
          ...
          <div className="bg-white rounded-2xl border border-slate-100 p-4 shadow-sm flex flex-col sm:flex-row gap-4 items-center justify-between">
            <h4 className="text-sm font-black text-slate-800 uppercase tracking-widest">
              Ưu đãi hiện có ({promoPackages.length})
            </h4>
                    ...
                    filteredPromos.map((promo) => (
                      ...
                      >
                        ...
                        <td className="px-6 py-4">
                          <span className="inline-flex px-2 py-0.5 rounded-lg bg-emerald-50 text-emerald-700 text-xs font-black">
                            -{promo.discountValue}
                          </span>
                        </td>
                          ...
                          <div className="inline-flex gap-2">
                            ...
                            <button
                              onClick={() => {
                                if (
                                  confirm(
                                    `Bạn có chắc chắn muốn xóa chương trình "${promo.name}"?`
                                  )
                                ) {
                                  removePromoPackage(promo.id);
                                }
                              }}
